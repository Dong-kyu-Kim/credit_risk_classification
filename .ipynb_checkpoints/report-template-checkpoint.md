...
The goal is to determine if Logistic Regression model can be accurate to predict healhy loans versus high risk loans using original data versus resampled data to increase the size of the minority class.
...
Overall Logistic Regression model performed well especially prediction the outcome 0 healthy loans for class 0 the both the prediction and recall were extremly high.

* Does performance depend on the problem we are trying to solve? (For example, is it more important to predict the `1`'s, or predict the `0`'s? )

For the 1 class high risk loans the model precision is 0.84 and the recall 0.91.
This indicate the model more often give false positives than false negatives.
